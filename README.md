...
If VS Code shows this squiggle before the dev container setup finishes, wait for setup to complete and run **Terminal > Run Task... > Pebble: Build** once. The repo includes `.vscode/c_cpp_properties.json` so the C/C++ extension can find Pebble SDK headers under `$HOME/.local/share/pebble-sdk/SDKs/current/`.
